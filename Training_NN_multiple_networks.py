...
lr_steps = int(config_data[20][1])
lr_stop = float(config_data[21][1])
network_multiplyer = config_data[22][1].split(']')
network_multiplyer = network_multiplyer[0].split('[')
network_multiplyer = network_multiplyer[1].split(',')
...
print("------------------------------------------------------\nLoading images...\n")
##----------------------------------------------------
## 2-Shuffle and split data in train and test set
##----------------------------------------------------
data_test = []
data_validat = []
data_train = []
...
#testSet = list(zip(data_test,labels_test))
#validatSet = list(zip(data_validat,labels_validat))
#trainSet = list(zip(data_train,labels_train))
#random.shuffle(testSet)
#random.shuffle(validatSet)
#random.shuffle(trainSet)
#data_test,labels_test = zip(*testSet)
#data_validat,labels_validat = zip(*validatSet)
#data_train,labels_train = zip(*trainSet)
...
# print("Validation data shape:",data_validat.shape)
# print("Validation labels shape:",labels_validat.shape)
...
##----------------------------------------------------
## 3-Create and configure CNN models
##----------------------------------------------------
...
if train_type == '1' or train_type == '3':
    ...
    print("------------------------------------------------------\nTraining color ANN...\n")
    plot_model(model_color, to_file='results/color/models/color_ANN_model.png',show_shapes=True,show_layer_names=False)
    ...
    model_color.save("results/color/models/color_ANN_model.h5")
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("results/color/color_accuracy_plot.png")
    ...
    for i in range(10):
        randNum = random.randint(1,631)
        print("Imagem escolhida: imagem",randNum)
        teste_image_path = ('database/image_'+str(randNum)+'.jpeg')
        teste_image = cv2.imread(teste_image_path,1)
        teste_image = cv2.resize(teste_image, (224, 224))
        cv2.imshow('Preview Image!',teste_image)
        waitInput = cv2.waitKey(0)
        cv2.destroyAllWindows()
        teste_image = np.array(teste_image) / 255.0
        teste_image = np.expand_dims(image,axis=0)
        print(teste_image)
        teste_color_predict = model_color.predict(teste_image)
        print(teste_color_predict)



if train_type == '2' or train_type == '3':
    for l in range(len(network_multiplyer)):
        number_filters_actual=number_filters.copy()
        for i in range(len(number_filters)):
            number_filters_actual[i] = int(number_filters[i])*int(network_multiplyer[l])
        log=open('results/quality/network'+str(l)+'/training_log'+str(l)+'.txt','a+')
        ...
        log.write("\ntrain_per = "+str(train_per)+"\nvalidat_per = "+str(validat_per)+"\ntest_per = "+str(test_per)+
                  ...
                  "\nrandom_seed = "+str(random_seed)+"\nlr_steps = "+str(lr_steps)+"\nlr_stop = "+str(lr_stop)+"\n\n")
        log.close()
        for i in range(0,(lr_steps+1)):
            date_time = datetime.datetime.now()
            lr_actual_quality=lr_inicial_quality-(i*(lr_inicial_quality-lr_stop)/lr_steps)
            log=open('results/quality/network'+str(l)+'/training_log'+str(l)+'.txt','a+')
            log.write("\n------------------------------------------------------\nStart time: {}\nQuality learning rate = {}\n".format(date_time.strftime("%d-%m-%Y %H:%M"),lr_inicial_quality))
            log.close()
            print("\n------------------------------------------------------\nSetting up quality CNN...\n")
            print("lr =",lr_actual_quality)
            model_quallity = Sequential()
            #[256x256x4] -> CONV2D + RELU + BatchNorm + POOL + DROP
            model_quallity.add(Conv2D(int(number_filters_actual[0]), filter_size, padding='same', activation='relu', input_shape=(image_size[0], image_size[1],3)))
            ...
            model_quallity.add(Conv2D(int(number_filters_actual[1]), filter_size, padding='same', activation='relu'))
            model_quallity.add(BatchNormalization())
            model_quallity.add(Conv2D(int(number_filters_actual[1]), filter_size, padding='same', activation='relu'))
            model_quallity.add(BatchNormalization())
            model_quallity.add(MaxPooling2D(pool_size=2))
            ...
            model_quallity.add(Conv2D(int(number_filters_actual[2]), filter_size, padding='same', activation='relu'))
            model_quallity.add(BatchNormalization())
            model_quallity.add(Conv2D(int(number_filters_actual[2]), filter_size, padding='same', activation='relu'))
            model_quallity.add(BatchNormalization())
            model_quallity.add(Conv2D(int(number_filters_actual[2]), filter_size, padding='same', activation='relu'))
            ...
            model_quallity.add(Conv2D(int(number_filters_actual[3]), filter_size, padding='same', activation='relu'))
            model_quallity.add(BatchNormalization())
            model_quallity.add(Conv2D(int(number_filters_actual[3]), filter_size, padding='same', activation='relu'))
            model_quallity.add(BatchNormalization())
            model_quallity.add(Conv2D(int(number_filters_actual[3]), filter_size, padding='same', activation='relu'))
            ...
            plot_model(model_quallity, to_file="results/quality/network"+str(l)+"/models/quality_CNN_model_"+str(l)+"_"+str(i)+".png",show_shapes=True,show_layer_names=False)
            ...
            qualityoptimizer = SGD(lr=lr_actual_quality, decay=lr_actual_quality/Epochs_quality)
            model_quallity.compile(loss="categorical_crossentropy", optimizer=qualityoptimizer, metrics=["accuracy"])
            qualityearlystop = EarlyStopping(monitor='val_loss', min_delta=0.01, patience=quality_earlystop_patience, verbose=1, mode='auto', restore_best_weights=True)
            quality_callbacks_list = [qualityearlystop]
              
              
            ##----------------------------------------------------
            ## 5-Training, save (model_quallity + label binarizer)
            ...
            model_quallity.save("results/quality/network"+str(l)+"/models/quality_CNN_model_"+str(l)+"_"+str(i)+".h5")
            ...
            plt.ylabel("Accuracy")
            plt.legend()
            plt.savefig("results/quality/network"+str(l)+"/quality_loss_plot_"+str(l)+"_"+str(i)+".png")
            
            ##----------------------------------------------------
            ## 7-Predict and evaluate for test dataset
            ##----------------------------------------------------
            date_time = datetime.datetime.now()
